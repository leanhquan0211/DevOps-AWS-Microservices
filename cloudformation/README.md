...
Thư mục này chứa phần triển khai hạ tầng AWS bằng CloudFormation cho bài thực hành DevOps. Toàn bộ quá trình kiểm tra, build và triển khai được thiết kế để chạy trên dịch vụ AWS, không dùng script local.
...
- chạy `cd cloudformation && taskcat test run`

File `.taskcat.yml` hiện nằm trực tiếp tại root của thư mục `cloudformation/`, vì vậy template path trong file được đặt là `templates/infrastructure.yaml`.

## Lưu ý về Key Pair

Trước khi chạy pipeline, cần kiểm tra giá trị `KeyName` trong `cloudformation/.taskcat.yml`.
